...
class SAC_RL:
    ...
    def preprocess_state(self, state_values):
        state = np.asarray(state_values, dtype=np.float32)

        if state.shape[0] != self.state_dim:
            raise ValueError(
                f"Expected {self.state_dim} state values, got {state.shape[0]}"
            )

        ir_state = np.clip(state[:8], 0, IR_MAX_VALUE) / IR_MAX_VALUE

        color_state = state[8:17].copy()

        # Each color has: seen, area, center_x
        for i in [0, 3, 6]:
            color_state[i] = np.clip(color_state[i], 0.0, 1.0)
            color_state[i + 1] = np.clip(color_state[i + 1], 0.0, 1.0)

            # Convert center_x from [-1, 1] to [0, 1]
            color_state[i + 2] = (np.clip(color_state[i + 2], -1.0, 1.0) + 1.0) / 2.0

        return np.concatenate([ir_state, color_state])
    ...
    def select_action(self, state_values, evaluate=False):
        ...
        state = self.preprocess_state(state_values)
